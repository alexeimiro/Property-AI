...
export function FilterSidebar({ filters, onFilterChange, isOpen, onToggle }: FilterSidebarProps) {
  ...
  const handleDistressTypeChange = (type: string, checked: boolean) => {
    setLocalFilters({
      ...localFilters,
  ...
  const handleBedroomsChange = (bedrooms: number, checked: boolean) => {
    setLocalFilters({
      ...localFilters,
      bedrooms: checked 
  ...
  const handleBathroomsChange = (bathrooms: number, checked: boolean) => {
    setLocalFilters({
      ...localFilters,
      bathrooms: checked 
  ...
  const handleInvestmentScoreChange = (value: number) => {
    setLocalFilters({
      ...localFilters,
              ...
              {["foreclosure", "short-sale", "reo", "probate", "auction", "other"].map((type) => (
                <div key={type} className="flex items-center space-x-2">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => handleDistressTypeChange(type, checked as boolean)}
                  />
                  <label
              ...
              {[1, 2, 3, 4, 5].map((beds) => (
                <div key={beds} className="flex items-center space-x-2">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => handleBedroomsChange(beds, checked as boolean)}
                  />
                  <label
                    htmlFor={`beds-${beds}`}
                    className="text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
              ...
              {[1, 1.5, 2, 2.5, 3, 3.5, 4].map((baths) => (
                <div key={baths} className="flex items-center space-x-2">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => handleBathroomsChange(baths, checked as boolean)}
                  />
                  <label
                    htmlFor={`baths-${baths}`}
                    className="text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
